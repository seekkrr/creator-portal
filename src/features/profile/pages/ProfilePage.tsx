...
function OnboardingChecklist({ onboarding }: { onboarding: CreatorOnboarding | undefined }) {
    const items = [
        ...
        { key: "first_quest_created", label: "Create your first quest", hint: "Build a quest as a draft" },
        { key: "payout_account_set", label: "Add a payout account", hint: "So you can get paid" },
        { key: "is_verified", label: "Get verified", hint: "Unlocks submitting quests for review" },
    ] as const;
    ...
    return (
        <Card padding="md" className="rounded-2xl">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm font-medium text-neutral-500">{done}/{items.length}</span>
            </div>
            <div className="h-2 w-full rounded-full bg-neutral-100 overflow-hidden mb-5" aria-hidden="true">
                <div
                ...
                {items.map((item) => {
                    ...
                    return (
                        <li key={item.key} className="flex items-start gap-3">
                            ...
                            <div>
                                <p className={`text-sm font-medium ${complete ? "text-neutral-400 line-through" : "text-neutral-900"}`}>
                                    {item.label}
                                </p>
...
export function ProfilePage() {
    ...
    const savedAvatar =
        user?.profile_image?.url ?? creator?.avatar_url ?? null;
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const nameDirty = firstName !== (user?.first_name ?? "") || lastName !== (user?.last_name ?? "");
            ...
            <Card padding="lg" className="rounded-2xl">
                <div className="flex flex-col sm:flex-row sm:items-center gap-5">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        {creator?.tagline && <p className="text-neutral-700 mt-2 italic">“{creator.tagline}”</p>}
                    </div>
                </div>
                ...
                {!verified && (
                    <div className="mt-5 flex items-start gap-3 rounded-xl bg-amber-50 border border-amber-100 p-4 text-amber-800">
                        ...
                        <p className="text-sm">
                            You're not verified yet. Keep building great draft quests — verification by our team
                            unlocks submitting quests for review and a Verified Creator badge.
                        </p>
                    </div>
                )}
            </Card>
                ...
                <Card padding="lg" className="lg:col-span-2 rounded-2xl">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center gap-3 pt-1">
                            ...
                            <Button variant="ghost" type="button" onClick={handleReset} disabled={!dirty || saving}>
                                Reset
                            </Button>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <Card padding="md" className="rounded-2xl">
                        ...
                        <div className="space-y-4">
                            <StatTile icon={<MapPin className="w-5 h-5" />} label="Quests" value={`${creator?.total_quests ?? 0}`} />
                            <StatTile icon={<Users className="w-5 h-5" />} label="Travelers served" value={`${creator?.travelers_served ?? 0}`} />
                            <StatTile icon={<Wallet className="w-5 h-5" />} label="Earnings" value={`₹${(creator?.total_earnings ?? 0).toLocaleString()}`} />
                            <StatTile
                                icon={<Star className="w-5 h-5" />}
                                label="Rating"
                                value={creator?.rating != null ? `${creator.rating.toFixed(1)} (${creator.review_count ?? 0})` : "—"}
                            />
                        </div>
                    </Card>
                </div>
