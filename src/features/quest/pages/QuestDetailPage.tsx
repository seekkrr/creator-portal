    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 max-w-[1100px] mx-auto space-y-6 sm:space-y-8 animate-fade-in pb-12 sm:pb-24">
                    ...
                    <Card className="w-full max-w-md shadow-2xl border-red-100 overflow-hidden animate-scale-up">
                        <div className="p-6">
                            ...
                            <p className="text-slate-600 mb-6">
                                This action will delete your quest. To confirm, please type <span className="font-bold text-slate-900 select-none">CONFIRM</span> below.
                            </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                <StatCard icon={<Star className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500" />} label="Difficulty" value={quest.metadata?.difficulty || "Medium"} />
                <StatCard icon={<Clock className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500" />} label="Duration" value={`${quest.metadata?.duration_minutes || 60} mins`} />
                <StatCard icon={<Tag className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-500" />} label="Theme" value={quest.metadata?.theme || "Culture"} />
                <StatCard icon={<Calendar className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500" />} label="Created" value={formatDate(quest.created_at)} />
            </div>
                ...
                <Card className="rounded-2xl border-amber-200 shadow-sm bg-white overflow-hidden">
                    <div className="p-2">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h2 className="text-sm font-bold text-amber-900 uppercase tracking-widest">Admin Feedback</h2>
                        </div>
                        <div className="flex flex-col gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                            {[...(quest.review_history || [])]
                                ...
                                .slice(0, 5)
                                .map((review, i) => (
                                    <div key={i} className="p-3 sm:p-4 rounded-xl border border-amber-100 shadow-sm shrink-0">
                                        <p className="text-slate-800 text-sm whitespace-pre-wrap mb-2 italic leading-relaxed">"{review.comment}"</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="p-2">
                            ...
                            <div className="space-y-3">
                                {quest.steps?.map((step, i) => (
                                    <div key={i} className="border border-slate-100 rounded-xl overflow-hidden bg-white hover:border-indigo-200 transition-colors shadow-sm">
                                        <button
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <h4 className="font-bold text-slate-800 group-hover:text-indigo-600 transition-colors text-sm sm:text-base leading-snug pr-4">{step.title}</h4>
                                            </div>
                                            <div className={`transition-transform duration-200 text-slate-400 shrink-0 ${expandedSteps.has(i) ? "rotate-180" : ""}`}>
                                            ...
                                            <div className="px-4 sm:px-5 pb-5 pt-0 sm:pl-16 animate-slide-down">
                                                ...
                                                {step.how_to_reach && (
                                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-4">
                                                        ...
                                                        <p className="text-sm text-slate-700 leading-relaxed">{step.how_to_reach}</p>
                                                    </div>
                                                )}
                                                ...
                                                {step.cloudinary_assets && step.cloudinary_assets.length > 0 && (
                                                    <div className="flex gap-3 mt-4 overflow-x-auto pb-3 custom-scrollbar">
                                                        {step.cloudinary_assets.map((asset, idx) => (
                                                            <div key={idx} className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden border border-slate-200 flex-shrink-0 bg-slate-100 group-scope relative shadow-sm">
                                                                <img src={asset.secure_url} alt="" className="w-full h-full object-cover transition-transform duration-500 hover:scale-110" />
                                                            </div>
                                                        ))}
                                                    </div>
                                                )}
                ...
                <div className="space-y-6 sm:space-y-8">
                        ...
                        <div className="p-2">
                            ...
                            {quest.media?.cloudinary_assets && quest.media.cloudinary_assets.length > 0 ? (
                                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                                    {quest.media.cloudinary_assets.map((asset, i) => (
                                        <div key={i} className="aspect-square rounded-xl overflow-hidden border border-slate-200 shadow-sm group cursor-pointer relative bg-slate-100">
                            ...
                            {quest.media?.reel_url && (
                                <div className="mt-6 bg-indigo-50/50 p-4 rounded-xl border border-indigo-100/50">
                                    <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-2">Source Reel</h4>
                                    <a
                    ...
                    <Card className="rounded-2xl border-slate-200 shadow-sm">
                        <div className="p-2">
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-6">Quest Info</h3>
                            <div className="space-y-1">
                                ...
                                <DetailItem label="Price" value={(quest.price ?? 0) > 0 ? `₹${quest.price.toLocaleString("en-IN")}` : "Free"} />
                            </div>
                        </div>
                    </Card>
                </div>
...
function DetailItem({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex justify-between items-center py-2.5 sm:py-3 border-b border-slate-100 last:border-0">
            ...
            <span className="text-sm text-slate-800 font-bold max-w-[50%] text-right truncate">{value}</span>
        </div>
    );
}
